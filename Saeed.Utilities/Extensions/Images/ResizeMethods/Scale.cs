...
namespace Saeed.Utilities.Extensions.Images.ResizeMethods
{
    ...
    public class Scale : IResizeMethod
    {
        ...
        {
            var orgRatio = imgSize.Width / (float)imgSize.Height;

            // if new image size is larger than original then use original image size 
            var _newWidth = (float)targetSize.Width == 0 ? targetSize.Height * orgRatio : targetSize.Width;

            var _newHeight = (float)targetSize.Height == 0 ? targetSize.Height / orgRatio : targetSize.Height;
